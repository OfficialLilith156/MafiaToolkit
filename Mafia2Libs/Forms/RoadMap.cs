    ...
    public partial class Form1 : Form
    {
        ...
        private void RefreshSplineList()
        {
            ...
            for (int i = 0; i < _roadmap.Splines.Count; i++)
                _listSplines.Items.Add($"Spline {i} (Points: {_roadmap.Splines[i].Points.Count})");
        }

        private void OnSplineSelected(object sender, EventArgs e)
        {
            if (_listSplines.SelectedIndex >= 0 && _listSplines.SelectedIndex < _roadmap.Splines.Count)
            {
                _currentSpline = _roadmap.Splines[_listSplines.SelectedIndex];
        ...
        {
            if (_roadmap == null || _roadmap.Splines.Count == 0)
            {
        ...
        private void OnCanvasPaint(object sender, PaintEventArgs e)
        {
            ...
            foreach (var spline in _roadmap.Splines)
            {
                ...
                Color lineColor = (spline == _currentSpline) ? Color.LimeGreen : Color.Gray;
        ...
        private void OnCanvasMouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left && _currentSpline != null)
            {
                float minDist = 10;
                _selectedPointIndex = -1;
                ...
                if (_selectedPointIndex >= 0)
                {
                    ...
                    _dragStartWorld = new Vector2(_currentSpline.Points[_selectedPointIndex].X, _currentSpline.Points[_selectedPointIndex].Z);
                    UpdateSelectedPointDisplay();
                    _canvas.Invalidate();
                    return;
                }
            ...
            {
                int splineIndex = FindSplineAtScreenPoint(new PointF(e.X, e.Y));
                if (splineIndex >= 0 && splineIndex < _roadmap.Splines.Count)
                {
                    _currentSpline = _roadmap.Splines[splineIndex];
                    _selectedPointIndex = -1;
                    _listSplines.SelectedIndex = splineIndex;
                    UpdateSelectedPointDisplay();
                    _canvas.Invalidate();
                }
            }
        ...
        private int FindSplineAtScreenPoint(PointF screenPoint)
        {
            int bestSplineIndex = -1;
        ...
        private void OnCanvasMouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (_selectedPointIndex >= 0 && e.Button == MouseButtons.Left && _currentSpline != null)
            {
                ...
                _currentSpline.Points[_selectedPointIndex] = point;
