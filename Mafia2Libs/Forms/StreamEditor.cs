...
"AREA565_OYSTERBAY_DLCCATHOUSE"
        ...
        {
            if (areaNameHashes.TryGetValue(hash64, out string name))
                return name;

            if (_knownHashes64.TryGetValue(hash64, out name))
                return name;

            uint hash32 = (uint)(hash64 & 0xFFFFFFFF);
            if (_knownHashes32.TryGetValue(hash32, out name))
                return name;
                ...
                if (block.Hashes != null && block.Hashes.Length > 0)
                {
                    ...
                    for (int j = 0; j < block.Hashes.Length; j++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(resolvedName))
                        {
                            ...
                            if (areaNameHashes.ContainsKey(hash))
                                source = "CityAreas";
